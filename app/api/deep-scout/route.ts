  ...
  try {
    ...
    const { object } = await generateObject({
      ...
      schema: z.object({
        ...
        // ★修正：analysisスキーマに3セクション出力を強制
        analysis: z.string().describe("推論プロセス。必ず【カタログからの推論】【自社実測データからの推論】【最終結論】の3セクションに分け、改行(\\n)を含めて出力すること。"),
...
【絶対ルール4：推論の優先順位と出力フォーマット】★新規追加
第1優先：当社の確定マスターデータ（現場のリアルな実測値）
第2優先：参考カタログデータ（理論上の数値）
第3優先：比重計算および物理的な面積比

analysis（推論プロセス）は、AIの思考プロセスを人間が検証できるよう、必ず以下の3セクションに分け、改行文字を入れて出力してください。
【カタログからの推論】 (例: カタログの〇〇○sqと外径が一致。理論歩留まりはX%)
【自社実測データからの推論】 (例: 当社マスターの類似品〇〇の実測データはY%)
【最終結論】 (例: 理論値はX%だが、当社の実測Y%を優先し、年代の古さを加味して最終的にZ%と判定)
...
4. 絶対ルール4に従い、3セクション構成で推論プロセスを記述せよ。
5. 算出した歩留まりを元に、「限界交渉アドバイス」を提示せよ。`
            },
