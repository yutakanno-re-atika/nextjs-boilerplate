...
export async function POST(req: Request) {
  try {
    ...
    const { object } = await generateObject({
      // ★ 修正: ビルドエラーを回避するため、オプションを外してモデルの変更（Pro）のみに留める
      model: google('gemini-2.5-pro'), 
      schema: z.object({
        targets: z.array(z.object({
          company: z.string().describe("【必須】企業名（完全に実在が確認できる企業のみ）"),
          address: z.string().describe("所在地（番地まで。架空の住所は不可）"),
          area: z.string().describe("エリア（例：苫小牧市）"),
          industry: z.string().describe("業種"),
          contact: z.string().describe("【必須】電話番号（公式サイト等で確認できる実在の番号のみ）"),
      ...
      prompt: `
        ...
        以下の条件に完全に合致する、**現在確実に実在し、コンタクト可能な企業**を${count || 5}件リストアップしてください。
        ${teacherContext}
        ...
        1. 検索と知識の統合: 企業名、住所、電話番号、WebサイトURLは、正確な情報を出力してください。存在しない架空の企業を作ったり、「http://www.example.com」や「0144-00-0000」などの推測データを出力した場合は致命的なエラーとみなします。
        2. 情報の完全性（除外ルール）: 連絡先（電話番号）とWebサイトURLが両方とも実在する企業【のみ】を抽出してください。どちらか一方でも不明な企業は絶対に含めないでください。見つからない場合は、抽出件数が${count || 5}件未満になっても構いません。
