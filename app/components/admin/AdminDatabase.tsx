    ...
    if (pendingItemStr) {
      try {
        ...
        setEditingItem({
            ...
            aiEstimatedRatio: pendingData.ratio || '',
  ...
  const renderModalContent = () => {
    if (activeTab === 'WIRES' || activeTab === 'UNKNOWN') return (
      <div className="space-y-4">
        ...
        <div className="mt-4"><label className="block text-xs font-bold text-gray-500 mb-1">メモ / 特記事項 (AIの推論根拠など)</label><textarea className="w-full border p-3 rounded-sm h-28 text-sm outline-none focus:border-gray-500 leading-relaxed shadow-sm bg-gray-50 whitespace-pre-wrap" value={editingItem.memo || editingItem.reason || ''} onChange={e => setEditingItem({...editingItem, memo: e.target.value, reason: e.target.value})} /></div>
      </div>
    );
