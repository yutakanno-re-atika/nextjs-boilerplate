...
<button onClick={runDataCleansing} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1.5 rounded-sm font-bold flex items-center gap-1 transition border border-gray-600 shadow-sm">
...
{CATEGORIES.map(cat => (
<button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-1 md:px-4 md:py-1.5 rounded-full text-[9px] md:text-[10px] font-bold whitespace-nowrap transition-colors border shadow-sm ${selectedCategory === cat ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-500 border-gray-300 hover:bg-gray-100 hover:text-gray-900'}`}>
{cat}
</button>
))}
</div>
...
<div key={m.id || i} onClick={() => handleOpenModal(m)} className="bg-white border border-gray-300 hover:border-blue-400 rounded-sm p-2 flex gap-3 w-64 shrink-0 relative shadow-sm cursor-pointer transition group">
...
{activeTab === 'CLIENTS' && (
<button onClick={() => { setEditingItem(item); handleEnrichClient(item); }} disabled={isSubmitting} className="p-2 text-yellow-600 hover:text-yellow-800 hover:bg-yellow-50 border border-transparent hover:border-yellow-200 rounded-sm transition shadow-sm" title="AIで企業情報を深掘り">
<Icons.Sparkles />
</button>
)}
...
{members.map((m: any, i: number) => (
<div key={m.id || i} onClick={() => handleOpenModal(m)} className="bg-white border border-gray-300 hover:border-blue-400 rounded-sm p-2 flex gap-3 relative shadow-sm cursor-pointer transition group">
{m.status === 'archived' && <span className="absolute top-0 right-0 bg-gray-200 text-gray-500 text-[8px] font-bold px-1.5 py-0.5 rounded-bl-sm z-10">Archived</span>}
{m.image1 ? (
<img src={getDriveImageUrl(m.image1)} className="w-14 h-14 object-cover border border-gray-200 rounded-sm shrink-0" />
) : (
...
<div className="flex gap-1.5">
<button onClick={() => { setEditingItem(item); handleEnrichClient(item); }} disabled={isSubmitting} className="p-1.5 text-yellow-600 bg-yellow-50 border border-yellow-200 rounded-sm hover:bg-yellow-100 transition" title="AIで企業情報を深掘り">
<Icons.Sparkles />
</button>
<button onClick={() => handleOpenModal(item)} className="p-1.5 text-gray-600 bg-gray-50 border border-gray-200 rounded-sm"><Icons.Edit /></button>
<button onClick={() => handleDelete(item.id)} className="p-1.5 text-[#D32F2F] bg-red-50 border border-red-100 rounded-sm"><Icons.Trash /></button>
...
<button onClick={() => handleEnrichClient()} disabled={isSubmitting} className="bg-yellow-50 text-yellow-800 border border-yellow-300 px-3 py-1.5 rounded-sm text-xs font-bold flex items-center gap-1 hover:bg-yellow-100 transition disabled:opacity-50">
...
<input type="number" step="0.001" className="w-full border border-gray-300 shadow-sm p-2.5 md:p-3 rounded-sm font-mono text-base md:text-xl outline-none focus:border-gray-900 text-right bg-white font-bold tabular-nums placeholder:text-gray-300 placeholder:text-xs" value={sampleCover} onChange={e => handleSampleCoverChange(e.target.value)} placeholder={sampleTotal && sampleCopper && !sampleCover ? `(自動) ${getAutoCoverWeight()}` : "0.000"} />
</div>
