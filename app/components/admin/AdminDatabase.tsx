...
export const AdminDatabase = ({ data, onNavigate }: { data: any, onNavigate?: any }) => {
    ...
    const formatUpdates = (sheetName: string, form: any) => {
        ...
        // ★ Wires の11項目対応 (0=ID, 1=maker, 2=name, 3=year, 4=sq, 5=sample_total, 6=sample_copper, 7=core, 8=conductor, 9=ratio, 10=memo)
    ...
    return (
        <div className="flex flex-col h-full animate-in fade-in duration-300 max-w-7xl mx-auto w-full pb-8 text-gray-800">
            <header className="mb-6 flex-shrink-0 border-b border-gray-200 pb-4 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="relative flex-1 md:w-64">
                        ...
                        <input 
                            type="text" 
                            className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-sm text-sm outline-none focus:border-[#D32F2F] shadow-inner" 
                            placeholder="全件検索..." 
                            value={searchTerm} 
                            onChange={(e) => setSearchTerm(e.target.value)} 
                        />
                    </div>
                    <button onClick={handleAddClick} disabled={isAdding || editingId !== null} className="bg-[#111] text-white px-5 py-2.5 rounded-sm text-sm font-bold hover:bg-[#D32F2F] transition shadow-sm flex justify-center items-center gap-1 disabled:opacity-50 flex-shrink-0">
                        <Icons.Plus /> 新規追加
                    </button>
                </div>
            </header>
            ...
            <div className="flex border-b border-gray-200 bg-gray-50 flex-shrink-0 overflow-x-auto no-scrollbar">
                <button onClick={() => handleTabChange('CLIENTS')} className={`px-5 py-3.5 text-xs font-bold tracking-widest whitespace-nowrap transition-colors ${activeTab === 'CLIENTS' ? 'bg-white border-t-2 border-t-[#D32F2F] text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}>👥 顧客マスター (CRM)</button>
                <button onClick={() => handleTabChange('SALES_TARGETS')} className={`px-5 py-3.5 text-xs font-bold tracking-widest whitespace-nowrap transition-colors ${activeTab === 'SALES_TARGETS' ? 'bg-white border-t-2 border-t-[#D32F2F] text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}>🎯 営業リスト</button>
                <button onClick={() => handleTabChange('WIRES')} className={`px-5 py-3.5 text-xs font-bold tracking-widest whitespace-nowrap transition-colors ${activeTab === 'WIRES' ? 'bg-white border-t-2 border-t-[#D32F2F] text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}>⚡ 電線マスター</button>
                <button onClick={() => handleTabChange('CASTINGS')} className={`px-5 py-3.5 text-xs font-bold tracking-widest whitespace-nowrap transition-colors ${activeTab === 'CASTINGS' ? 'bg-white border-t-2 border-t-[#D32F2F] text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}>⚙️ 非鉄マスター</button>
                <button onClick={() => handleTabChange('STAFF')} className={`px-5 py-3.5 text-xs font-bold tracking-widest whitespace-nowrap transition-colors ${activeTab === 'STAFF' ? 'bg-white border-t-2 border-t-[#D32F2F] text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}>⏱️ STAFF・労務費</button>
            </div>
                ...
                <div className="overflow-x-auto overflow-y-auto flex-1 p-0">
                    <table className="w-full text-left border-collapse min-w-[1000px]">
                        <thead className="sticky top-0 bg-gray-100 shadow-sm z-10 border-b border-gray-200">
                            <tr>
                                {activeTab === 'CLIENTS' && (
                                    <><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[25%]">顧客名・社名</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[15%]">ランク / Pt</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[15%]">連絡先</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[15%]">ログイン情報</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[20%]">メモ</th></>
                                )}
                                {activeTab === 'SALES_TARGETS' && (
                                    <><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[25%]">企業名</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[15%]">エリア / 優先度</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[15%]">連絡先</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[15%]">ステータス</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[20%]">メモ / 提案内容</th></>
                                )}
                                {activeTab === 'WIRES' && (
                                    <><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[35%]">メーカー / 品名 / 製造年 / 仕様</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[30%] text-center">サンプル総量 → 銅量 = 歩留%</th><th className="p-3 text-xs font-bold text-red-600 bg-red-50 uppercase w-[25%] text-right border-l border-red-100">💰 想定単価 (参考)</th></>
                                )}
                                {activeTab === 'CASTINGS' && (
                                    <><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[30%]">品名</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[20%]">種別 (相場)</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[20%] text-center">歩留まり (%)</th><th className="p-3 text-xs font-bold text-red-600 bg-red-50 uppercase w-[20%] text-right border-l border-red-100">💰 想定単価 (参考)</th></>
                                )}
                                {activeTab === 'STAFF' && (
                                    <><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[30%]">従業員名</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[25%]">役割 (Role)</th><th className="p-3 text-xs font-bold text-blue-600 bg-blue-50 uppercase w-[25%] text-right border-l border-blue-100">⏱️ 分間チャージレート</th><th className="p-3 text-xs font-bold text-gray-500 uppercase w-[10%] text-center">状態</th></>
                                )}
                                <th className="p-3 text-xs font-bold text-gray-500 uppercase w-[10%] text-right">操作</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 text-sm">
                                ...
                                <tr className="bg-blue-50/30">
                                    ...
                                    {activeTab === 'SALES_TARGETS' && (
                                        <>
                                            ...
                                            <td className="p-3 space-y-2"><input placeholder="エリア" className={inputClass} onChange={e => setFormState({...formState, area: e.target.value})} /><select className={inputClass} onChange={e => setFormState({...formState, priority: e.target.value})} defaultValue="B"><option value="A">A</option><option value="B">B</option><option value="C">C</option></select></td>
                                    ...
                                    {activeTab === 'WIRES' && (
                                        <>
                                            <td className="p-3 space-y-2">
                                                <div className="flex gap-2"><input autoFocus placeholder="メーカー" className={inputClass} onChange={e => setFormState({...formState, maker: e.target.value})} /><input placeholder="品名 (例: IV)" className={inputClass} onChange={e => setFormState({...formState, name: e.target.value})} /></div>
                                            ...
                                            <td className="p-3 text-center space-y-2 bg-gray-50 border-x border-gray-200">
                                                <div className="flex items-center gap-2 justify-center">
                                                    <input type="number" placeholder="総重量(g)" className={inputClass + " w-24 text-right"} onChange={e => handleSampleChange('sample_total', e.target.value)} />
                                                    <span className="text-gray-400 font-bold">→</span>
                                                    <input type="number" placeholder="銅重量(g)" className={inputClass + " w-24 text-right"} onChange={e => handleSampleChange('sample_copper', e.target.value)} />
                                                </div>
                                                <div className="flex items-center justify-center gap-2">
                                                    <span className="text-xs font-bold text-gray-500">歩留=</span>
                                                    <input type="number" className={`${inputClass} w-24 text-center font-black text-[#D32F2F]`} value={formState.ratio || ''} onChange={e => setFormState({...formState, ratio: e.target.value})} />
                                                    <span className="text-xs font-bold text-gray-500">%</span>
                                                </div>
                                            </td>
                                            <td className="p-3 bg-red-50 text-right font-mono text-gray-500 text-xs flex items-center justify-end h-full">保存後に計算</td>
                                        </>
                                    )}
                                    {activeTab === 'CASTINGS' && (
                                        <>
                                    ...
                                    {activeTab === 'STAFF' && (
                                        <>
                                            ...
                                            <td className="p-3 bg-blue-50 relative"><input type="number" inputMode="decimal" placeholder="50" className={`${inputClass} text-right font-black text-blue-700 pr-8 bg-white/50`} onChange={e => setFormState({...formState, rate: e.target.value})} /><span className="absolute right-6 top-1/2 -translate-y-1/2 text-sm text-gray-500">円</span></td>
                                    ...
                                    <td className="p-3 text-right align-top">
                                        <div className="flex flex-col gap-2 items-end">
                                            <button onClick={() => handleSave(true)} disabled={isSaving} className="bg-gray-900 text-white w-full py-2 rounded-sm text-xs font-bold hover:bg-black transition">{isSaving ? '処理中' : <><Icons.Save />保存</>}</button>
                                            <button onClick={() => setIsAdding(false)} className="bg-white border border-gray-300 text-gray-700 w-full py-2 rounded-sm text-xs font-bold hover:bg-gray-50 transition">取消</button>
                                        </div>
                                    </td>
                                </tr>
                            )}
                            ...
                            {filteredData.map((record: any) => {
                                ...
                                // ★ Reactのバグ防止：keyにactiveTabを含めてDOM再利用をブロック
                                return (
                                <tr key={`${activeTab}-${record.id}`} className="hover:bg-gray-50 transition">
                                    ...
                                    {editingId === record.id ? (
                                        <>
                                            ...
                                            {activeTab === 'WIRES' && (
                                                <>
                                                    <td className="p-3 space-y-2 bg-blue-50/20">
                                                        ...
                                                        <div className="flex gap-2"><input type="text" placeholder="製造年" className={inputClass} value={formState.year || ''} onChange={e => setFormState({...formState, year: e.target.value})} /><input type="text" placeholder="sq" className={inputClass} value={formState.sq || ''} onChange={e => setFormState({...formState, sq: e.target.value})} /><input type="text" placeholder="芯" className={inputClass} value={formState.core || ''} onChange={e => setFormState({...formState, core: e.target.value})} /><input type="text" placeholder="導体" className={inputClass} value={formState.conductor || ''} onChange={e => setFormState({...formState, conductor: e.target.value})} /></div>
                                                    ...
                                                    <td className="p-3 text-center space-y-2 bg-gray-50 border-x border-gray-200">
                                                        <div className="flex items-center gap-2 justify-center">
                                                            <input type="number" placeholder="総量" className={inputClass + " w-24 text-right"} value={formState.sample_total || ''} onChange={e => handleSampleChange('sample_total', e.target.value)} />
                                                            <span className="text-gray-400 font-bold">→</span>
                                                            <input type="number" placeholder="銅量" className={inputClass + " w-24 text-right"} value={formState.sample_copper || ''} onChange={e => handleSampleChange('sample_copper', e.target.value)} />
                                                        </div>
                                                        <div className="flex items-center justify-center gap-2">
                                                            <span className="text-xs font-bold text-gray-500">歩留=</span>
                                                            <input type="number" inputMode="decimal" className={`${inputClass} w-24 text-center font-black text-[#D32F2F]`} value={formState.ratio || 0} onChange={e => setFormState({...formState, ratio: e.target.value})} />
                                                            <span className="text-xs font-bold text-gray-500">%</span>
                                                        </div>
                                                    </td>
                                                    <td className="p-3 bg-red-50 border-l border-red-100 text-right"><span className="text-lg font-black font-mono text-red-600">¥{estimatedPrice.toLocaleString()}</span></td>
                                                </>
                                            )}
                                            {activeTab === 'CASTINGS' && (
                                                <>
                                            ...
                                            {activeTab === 'STAFF' && (
                                                <>
                                                    ...
                                                    <td className="p-3 bg-blue-50 relative border-l border-blue-100"><input type="number" inputMode="decimal" className={`${inputClass} text-right font-black text-blue-700 pr-8 bg-white/50`} value={formState.rate || 0} onChange={e => setFormState({...formState, rate: e.target.value})} /><span className="absolute right-6 top-1/2 -translate-y-1/2 text-sm text-gray-500">円</span></td>
                                            ...
                                            <td className="p-3 text-right align-top">
                                                <div className="flex flex-col gap-2 items-end">
                                                    <button onClick={() => handleSave(false, record.id)} disabled={isSaving} className="bg-gray-900 text-white w-full py-2 rounded-sm text-xs font-bold hover:bg-black transition">{isSaving ? '処理中' : <><Icons.Save />保存</>}</button>
                                                    <button onClick={() => setEditingId(null)} className="bg-white border border-gray-300 text-gray-700 w-full py-2 rounded-sm text-xs font-bold hover:bg-gray-50 transition">取消</button>
                                                </div>
                                            </td>
                                        </>
                                    ) : (
                                            ...
                                            {activeTab === 'CLIENTS' && (
                                                <>
                                                    <td className="p-4 align-top">
                                                        <div className="font-bold text-gray-900 text-base">{record.name}</div>
                                                    </td>
                                            ...
                                            {activeTab === 'WIRES' && (
                                                <>
                                                    <td className="p-4 align-top">
                                                        ...
                                                        <div className="flex gap-2 text-xs font-mono text-gray-500 mb-1">
                                                            {record.year && record.year !== '-' && <span className="bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">{record.year}年製</span>}
                                                            {record.sq && record.sq !== '-' && <span className="bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">{record.sq}sq</span>}
                                                            {record.core && record.core !== '-' && <span className="bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">{record.core}C</span>}
                                                            {record.conductor && record.conductor !== '-' && <span className="bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">{record.conductor}</span>}
                                                        </div>
                                                        {record.memo && <div className="text-xs text-gray-400 italic mt-1">{record.memo}</div>}
                                                    </td>
                                                    <td className="p-4 align-top text-center border-x border-gray-100 bg-gray-50/50">
                                                        <div className="text-xs text-gray-500 font-mono mb-1">
                                                            {record.sample_total > 0 ? `${record.sample_total}g → ${record.sample_copper}g` : '実測データなし'}
                                                        </div>
                                                        <span className="text-lg font-mono font-black text-gray-800">{record.ratio}%</span>
                                                    </td>
                                                    <td className="p-4 align-top bg-red-50/30 border-l border-red-50 text-right"><span className="text-xl font-black font-mono text-[#D32F2F]">¥{estimatedPrice.toLocaleString()}</span></td>
                                                </>
                                            )}
                                            {activeTab === 'CASTINGS' && (
                                                <>
                                                    <td className="p-4 align-top font-bold text-gray-900 text-base">{record.name}</td>
                                                    <td className="p-4 align-top"><span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-sm border border-gray-200 text-xs font-mono font-bold">{record.type}</span></td>
                                                    <td className="p-4 align-top text-center"><span className="text-lg font-mono font-black text-gray-700">{record.ratio}%</span></td>
                                                    <td className="p-4 align-top bg-red-50/30 border-l border-red-50 text-right"><span className="text-xl font-black font-mono text-[#D32F2F]">¥{estimatedPrice.toLocaleString()}</span></td>
                                                </>
                                            )}
                                            {activeTab === 'STAFF' && (
                                                <>
                                                    <td className="p-4 align-top font-bold text-gray-900 text-base">{record.name}</td>
                                                    <td className="p-4 align-top"><span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-sm border border-gray-200 text-xs font-mono font-bold">{record.role}</span></td>
                                                    <td className="p-4 align-top bg-blue-50/30 border-l border-blue-50 text-right">
                                                        <span className="text-xl font-black font-mono text-blue-700">{record.rate}</span>
                                                        <span className="text-xs text-gray-500 ml-1">円/分</span>
                                                        <div className="text-xs text-gray-400 mt-1 font-mono">(時給換算: {(record.rate * 60).toLocaleString()}円)</div>
                                                    </td>
                                                    <td className="p-4 align-top text-center">
                                                        <span className={`text-xs font-bold px-3 py-1 rounded-full ${record.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>{record.status}</span>
                                                    </td>
                                                </>
                                            )}
                                            <td className="p-4 align-top text-right">
                                                <div className="flex flex-col gap-2 items-end">
                                                    <button onClick={() => handleEditClick(record)} className="text-xs font-bold text-gray-500 border border-gray-300 px-3 py-1.5 hover:border-gray-900 hover:text-gray-900 transition rounded-sm w-full text-center">編集</button>
                                                    <button onClick={() => handleDelete(record.id)} className="text-xs font-bold text-gray-400 hover:text-[#D32F2F] transition w-full text-center py-1">削除</button>
                                                </div>
                                            </td>
                                        </>
                                    )}
                            ...
                            {filteredData.length === 0 && !isAdding && (
                                <tr>
                                    <td colSpan={6} className="py-16 text-center text-gray-400 font-bold text-base">
