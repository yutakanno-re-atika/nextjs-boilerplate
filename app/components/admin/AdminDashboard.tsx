         ...
         {adminTab === 'HOME' && (
             <div className="max-w-5xl mx-auto w-full animate-in fade-in zoom-in-95 duration-300 flex flex-col h-full">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 flex-shrink-0">
                     <button onClick={()=>setAdminTab('POS')} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:border-[#D32F2F] hover:shadow-md transition text-left flex items-start gap-4 group">
                         ...
                         <div>
                             <h3 className="text-xl font-bold text-gray-900 mb-1">受付・買取フロント</h3>
                             <p className="text-xs text-gray-500 leading-relaxed">ご来店いただいたお客様の情報と品物を登録し、明細を発行します。（会員DB連動）</p>
                         </div>
                     </button>
                     <button onClick={()=>setAdminTab('OPERATIONS')} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:border-orange-500 hover:shadow-md transition text-left flex items-start gap-4 group">
                         ...
                         <div>
                             <h3 className="text-xl font-bold text-gray-900 mb-1">現場カンバン (進行状況)</h3>
                             <p className="text-xs text-gray-500 leading-relaxed">予約の確認、計量中の荷物、加工待ちのリストをリアルタイムに確認・移動させます。</p>
                         </div>
                     </button>
                 </div>
                 ...
                 <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="bg-gray-50 p-4 rounded-xl">
                            <p className="text-[10px] text-gray-500 font-bold mb-1">来場予定・受付済</p>
                            <p className="text-2xl font-black text-gray-900">{reservedList.length} <span className="text-xs font-normal text-gray-500">件</span></p>
                        </div>
                        <div className="bg-red-50 p-4 rounded-xl border border-red-100">
                            <p className="text-[10px] text-[#D32F2F] font-bold mb-1">現在 検収・計量中</p>
                            <p className="text-2xl font-black text-[#D32F2F]">{processingList.length} <span className="text-xs font-normal text-red-300">件</span></p>
                        </div>
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                            <p className="text-[10px] text-blue-600 font-bold mb-1">ナゲット加工待ち</p>
                            <p className="text-2xl font-black text-blue-600">{completedList.length} <span className="text-xs font-normal text-blue-300">件</span></p>
                        </div>
                    </div>
                 </div>
             </div>
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-6 min-h-0">
                 <div className="space-y-4 overflow-y-auto pr-2 pb-4">
                    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
                       ...
                       <div className="grid grid-cols-2 gap-3 mb-3">
                           <div className="col-span-2 md:col-span-1">
                               ...
                               <input list="client-list" className="w-full bg-gray-50 border border-gray-200 p-3 rounded-lg text-gray-900 text-sm focus:border-[#D32F2F] outline-none transition font-bold" placeholder="名前を入力..." value={posCompany} onChange={(e)=>setPosCompany(e.target.value)} />
                    ...
                    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
                       ...
                       <h3 className="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2"><span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-xs">STEP 2</span> 品目の登録</h3>
                       <div className="flex flex-col md:flex-row gap-3 items-end">
                           ...
                           <div className="w-full md:w-20">
                               ...
                               <select className="w-full bg-gray-50 border border-gray-200 p-3 rounded-lg text-gray-900 text-sm font-bold outline-none" value={currentRank} onChange={(e:any)=>setCurrentRank(e.target.value)}>
                                  <option value="B">普</option><option value="A">良</option><option value="C">劣</option>
                               </select>
                           </div>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-lg h-full flex flex-col relative overflow-hidden">
                       ...
                       <div className="mb-2 flex-shrink-0">
                           <div className="flex justify-between items-start">
                               ...
                               <div className="text-right"><p className="text-[10px] text-gray-400 font-bold mb-0.5">受付</p><p className="text-sm font-bold text-gray-900">{posDate ? posDate.replace('T', ' ') : '本日 (飛込)'}</p></div>
                           </div>
                       </div>
                       
                       <div className="flex-1 overflow-y-auto space-y-2 border-t border-b border-gray-100 py-3 min-h-[150px]">
                           {cartItems.length === 0 ? <p className="text-center text-gray-400 text-sm mt-10">品物なし</p> : cartItems.map((item) => (
                               <div key={item.id} className="bg-gray-50 p-3 rounded-lg border border-gray-100 flex justify-between items-center">
                                   <div className="flex-1"><p className="text-sm font-bold text-gray-900 flex items-center gap-2">{item.productName}<span className="text-[9px] font-mono text-gray-400 border px-1 rounded">R:{item.rank}</span></p><p className="text-xs text-gray-500 font-mono mt-1">{item.weight} kg</p></div>
                       ...
                       <div className="pt-4 mt-2 flex-shrink-0">
                          ...
                          <button onClick={handleSubmitReservation} disabled={cartItems.length === 0 || !posCompany || isSubmitting} className="w-full bg-[#D32F2F] text-white py-3.5 rounded-xl font-bold hover:bg-red-700 transition shadow-md disabled:bg-gray-300 flex justify-center items-center gap-2">
                              {isSubmitting ? <span className="animate-pulse">送信中...</span> : <><Icons.Check /> 受付を完了して現場へ送る</>}
                          </button>
                       </div>
                    </div>
                 </div>
