...
import { AdminClientDetail } from './AdminClientDetail'; // ★ 追加
...
export const AdminDashboard = ({ data, setView, onLogout }: { data: any; setView: any; onLogout?: any }) => {
  const [adminTab, setAdminTab] = useState<'HOME' | 'OPERATIONS' | 'POS' | 'PRODUCTION' | 'COMPETITOR' | 'DATABASE' | 'CLIENT_DETAIL'>('HOME');
  ...
  const handleNavigate = (tab: any, id?: string) => {
      if (tab === 'POS' && id) setEditingResId(id);
      else setEditingResId(null);
      
      if (tab === 'CLIENT_DETAIL' && id) setSelectedClientName(id);
  ...
  return (
    <div className="h-screen w-full bg-[#F5F5F7] text-gray-900 font-sans flex flex-col md:flex-row overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col relative">
         ...
         {/* ★ 個別カルテ画面 */}
         {adminTab === 'CLIENT_DETAIL' && selectedClientName && (
             <AdminClientDetail data={data} clientName={selectedClientName} onBack={() => handleNavigate('HOME')} />
         )}
      </main>
    </div>
  );
