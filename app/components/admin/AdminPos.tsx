...
const runAiAnalysis = async () => {
  ...
  const startVoiceInput = () => {
      ...
      // iOSなどで無音が続くとすぐ切れるのを防ぐため、あえて1回の認識に絞る
      recognition.interimResults = false;
      recognition.maxAlternatives = 1;

      recognition.onstart = () => {
          setIsListening(true);
          setVoiceText('🎤 途切れないように一気に喋ってください...');
      };
  ...
  const processVoiceCommand = async (text: string) => {
      ...
      try {
          const res = await fetch('/api/gas', {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ action: 'PARSE_VOICE_COMMAND', text: text })
          });
          const result = await res.json();
          ...
          if (result.status === 'success' && result.parsed) {
              ...
              const matchedWire = (data?.wires || []).find((w: any) => {
                  ...
                  const mSq = String(w.sq || w.size || '');
                  const mCore = String(w.core || w.cores || '');
              ...
              if (matchedWire) {
                  addToCart(matchedWire, p.weight || 0);
                  setVoiceText(`追加完了: ${buildProductName(matchedWire)} ${p.weight ? p.weight+'kg' : ''}`);
              } else {
                  setVoiceText('該当するマスターが見つかりませんでした。');
              }
          } else {
              setVoiceText('聞き取れませんでした。もう一度お試しください。');
          }
      } catch (e) {
          setVoiceText('通信エラーが発生しました。');
      }
...
return (
    <div className="flex flex-col lg:flex-row gap-4 h-auto lg:h-full animate-in fade-in duration-300 pb-24 lg:pb-0 relative">
      ...
      {/* ★ トースト通知 (画面上部にフワッと出る) */}
      {toastMessage && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-top-10 fade-in duration-300 w-[90%] max-w-md">
          ...
          <div className="bg-gray-900 w-full max-w-xl rounded-md shadow-2xl animate-in zoom-in-95 border border-gray-700 overflow-hidden flex flex-col">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-300 mb-6 leading-relaxed">
                ...
                <span className="text-blue-400 text-xs">※既存品と一致した場合、マスターに画像がない場合は自動的に画像を補完登録します。</span>
              </p>
