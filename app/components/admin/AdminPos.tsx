...
export const AdminPos = ({ data, editingResId, localReservations, onSuccess, onClear }: { data: any; editingResId: string | null; localReservations: any[]; onSuccess: () => void; onClear: () => void; }) => {
  ...
  // デザイン: 入力フィールドの共通スタイル
  const inputClass = "w-full bg-gray-50 border border-gray-200 p-3 rounded-xl text-lg font-bold text-gray-900 outline-none focus:bg-white focus:border-[#D32F2F] focus:ring-2 focus:ring-red-100 transition shadow-inner";
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500 max-w-6xl mx-auto w-full">
      <header className="mb-6 flex justify-between items-end flex-shrink-0">
        <div>
          <h2 className="text-3xl font-serif font-bold text-gray-900 tracking-tight">{editingResId ? '検収・計量' : '新規受付'}</h2>
          <p className="text-sm text-gray-500 mt-1">POS System</p>
        </div>
        {editingResId && (
            <button onClick={onClear} className="text-gray-500 hover:text-gray-900 bg-white border border-gray-200 px-4 py-2 rounded-lg text-sm font-bold shadow-sm transition flex items-center gap-2">
                <Icons.Close /> 閉じる
            </button>
        )}
      </header>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-xl overflow-hidden flex flex-col flex-1">
          {/* 顧客情報エリア */}
          <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row gap-6">
              <div className="flex-1 relative">
                  ...
                  <div className="relative">
                      ...
                      <input 
                          type="text" 
                          className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl text-xl font-bold outline-none focus:border-[#D32F2F] focus:ring-4 focus:ring-red-500/10 transition shadow-sm" 
                          placeholder="業者名を検索..." 
                          value={clientName} 
                      ...
                      {showSuggest && clientName && (
                          <ul className="absolute z-20 w-full bg-white border border-gray-200 mt-2 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
              ...
              <div className="md:w-1/3 w-full">
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-2">Memo</label>
                  <input type="text" className="w-full border border-gray-300 p-3 rounded-xl text-base outline-none focus:border-[#D32F2F] transition" placeholder="備考" value={memo} onChange={e => setMemo(e.target.value)} />
              </div>
          </div>

          {/* 明細エリア */}
          <div className="p-6 flex-1 overflow-y-auto bg-white">
              <div className="hidden md:flex text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-2">
                  <div className="flex-1">Item</div>
                  <div className="w-36 text-right">Weight</div>
              ...
              <div className="space-y-3">
                  {items.map((item, idx) => (
                      <div key={idx} className="flex flex-col md:flex-row gap-3 md:items-center bg-gray-50 p-3 rounded-xl border border-gray-100 shadow-sm group hover:border-gray-300 transition-all duration-200">
                          <div className="flex-1 w-full md:w-auto">
                              <select className="w-full bg-transparent p-2 text-lg font-bold text-gray-900 outline-none cursor-pointer" value={item.product} onChange={e => handleItemChange(idx, 'product', e.target.value)}>
                          ...
                          <div className="w-12 text-right md:text-center">
                              <button onClick={() => removeItem(idx)} className="text-gray-300 hover:text-red-500 transition p-2 rounded-full hover:bg-red-50"><Icons.Trash /></button>
                          </div>
                      </div>
                  ))}
              </div>
              
              <button onClick={addItem} className="mt-6 w-full py-4 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 font-bold hover:border-[#D32F2F] hover:text-[#D32F2F] hover:bg-red-50 transition flex items-center justify-center gap-2">
                  <Icons.Plus /> 品目を追加
              </button>
          </div>

          {/* フッター（合計＆保存） */}
          <div className="p-6 bg-gray-900 text-white flex flex-col md:flex-row justify-between items-center gap-6 shadow-inner">
              <div className="text-center md:text-left">
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Total Payment</p>
                  <p className="text-5xl font-black tracking-tighter">¥{totalAmount.toLocaleString()}</p>
              </div>
              <button onClick={handleSubmit} disabled={isSubmitting || !clientName} className="w-full md:w-auto bg-[#D32F2F] text-white px-10 py-4 rounded-full text-lg font-bold hover:bg-red-600 transition shadow-lg flex items-center justify-center gap-3 disabled:bg-gray-700 disabled:text-gray-500 transform active:scale-95">
