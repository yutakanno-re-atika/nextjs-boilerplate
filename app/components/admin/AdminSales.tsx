...
export const AdminSales = ({ data }: { data: any }) => {
  ...
  const handleFetchGBizData = async () => {
      if (!apiToken) return alert('APIトークンを入力してください。');
    ...
    <div className="animate-in fade-in duration-500 max-w-7xl mx-auto w-full pb-20">
      ...
      <div className="mb-8 bg-white border border-gray-300 p-6 rounded-sm shadow-md relative overflow-hidden">
          <h3 className="font-bold flex items-center gap-2 mb-4 text-gray-900 text-lg">
              <Icons.Database /> SPARQL 高度絞り込みエンジン
          </h3>
          <p className="text-xs text-gray-600 mb-6 leading-relaxed">
              経産省のSPARQLエンドポイントに直接SQL構文を送信し、「指定エリア」かつ「法人名または事業概要にキーワードが含まれる」法人だけを1件の取りこぼしなく抽出します。
          </p>
          
          <div className="mb-4">
              <label className="block text-xs font-bold text-gray-700 mb-1">gBizINFO API トークン</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1">対象エリア</label>
              ...
              <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1">検索キーワード (法人名・事業内容)</label>
                  <div className="relative">
          ...
          <div className="flex justify-end border-t border-gray-100 pt-4">
              ...
              >
                  {isFetching ? <><Icons.Refresh /> SPARQLクエリ実行中...</> : <><Icons.Download /> エリアと業種で完全にぶっこ抜く</>}
              </button>
          </div>
      </div>
      ...
      <div className="bg-gray-50 p-4 border border-gray-200 rounded-sm flex gap-4 mb-6 shadow-sm">
        <div className="flex-1 relative">
            ...
            <input type="text" placeholder="取得済みの企業名、法人番号で検索..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-sm text-sm focus:outline-none focus:border-blue-700" />
        </div>
      </div>
                      ...
                      <div className="bg-gray-50 border-b border-gray-200 p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div>
                              <div className="flex items-center gap-2 mb-1">
                                  <span className="bg-green-700 text-white px-1.5 py-0.5 rounded-sm text-[9px] font-mono font-bold tracking-widest"><Icons.CheckCircle /> 国の公式データ</span>
                                  <span className="text-[10px] text-gray-500 font-mono">法人番号: {t.corporateNumber}</span>
                              </div>
                              <h3 className="text-lg font-black text-gray-900">{t.company}</h3>
                          </div>
                          <button onClick={() => handleDeleteTarget(t.id)} className="text-xs bg-white text-red-600 border border-red-200 px-3 py-2 rounded-sm font-bold shadow-sm hover:bg-red-50 hover:border-red-300 transition flex items-center gap-1 shrink-0"><Icons.Trash /> 削除</button>
                      </div>
                      <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-white">
                          <div className="col-span-1 md:col-span-2"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">所在地</p><p className="text-sm text-gray-800 font-bold">{t.address || '-'}</p></div>
                          <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">代表者名</p><p className="text-sm text-gray-800 font-bold">{t.representative || '-'}</p></div>
                          <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">資本金</p><p className="text-sm text-gray-800 font-bold">{t.capital || '-'}</p></div>
                          <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">従業員数</p><p className="text-sm text-gray-800 font-bold">{t.employees || '-'}</p></div>
                          <div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">設立/創業</p><p className="text-sm text-gray-800 font-bold">{t.founded || '-'}</p></div>
                          <div className="col-span-1 md:col-span-2"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Webサイト</p><p className="text-sm font-mono text-blue-600 break-all">{t.website || '-'}</p></div>
                          <div className="col-span-1 md:col-span-2 lg:col-span-4"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">事業概要 (事業内容から「{t.industry}」で抽出)</p><p className="text-xs text-gray-700 leading-relaxed bg-gray-50 p-3 rounded-sm border border-gray-100">{t.businessSummary || '情報なし'}</p></div>
                      </div>
                  </div>
              ))
          )}
