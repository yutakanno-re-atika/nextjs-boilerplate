...
export const AdminSales = ({ data }: { data: any }) => {
  ...
  const handleFetchGBizData = async () => {
      ...
      try {
          const payload = { area: targetArea, keyword: targetKeyword };
    ...
    <div className="animate-in fade-in duration-500 max-w-7xl mx-auto w-full pb-20">
      <header className="mb-6 flex flex-col md:flex-row md:justify-between md:items-end gap-4 border-b border-gray-200 pb-4">
        <div>
            <h2 className="text-2xl font-black text-gray-900 flex items-center gap-2 font-serif">
                ...
                法人データベース取得ツール (gBizINFO直結)
            </h2>
            <p className="text-xs text-gray-500 mt-1 font-mono tracking-wider ml-3">DATABASE EXTRACTOR / RAG PREPARATION</p>
        </div>
      </header>
      ...
      <div className="mb-8 bg-white border border-gray-300 p-6 rounded-sm shadow-md relative overflow-hidden">
          ...
          <p className="text-xs text-gray-600 mb-6 max-w-3xl leading-relaxed">
              入力されたキーワードとエリアに基づき、経済産業省のデータベース(gBizINFO API)へ直接リクエストを送信します。取得した公式法人データ（資本金、設立年など）は、AIを介さずそのまま自社のデータベース（スプレッドシート）に一括保存されます。
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1">検索キーワード (法人名・業種など)</label>
                  <div className="relative">
          ...
          <div className="flex justify-end border-t border-gray-100 pt-4">
              <button 
                  ...
                  disabled={isFetching || !targetArea || !targetKeyword}
              ...
              >
                  {isFetching ? <><Icons.Refresh /> データベースと通信・保存中 (最大60秒)...</> : <><Icons.Download /> 指定エリアの企業をDBにぶっこ抜く</>}
              </button>
          </div>
      </div>
                      ...
                      <div className="bg-gray-50 border-b border-gray-200 p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div>
                              <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <span className="bg-gray-200 text-gray-700 px-2 py-0.5 rounded-sm text-[10px] font-bold">{t.status}</span>
                              </div>
