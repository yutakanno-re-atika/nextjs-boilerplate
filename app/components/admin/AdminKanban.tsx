  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-300">
      <header className="mb-6 flex justify-between items-center flex-shrink-0">
        <div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tight">フロント・カンバン</h2>
          <p className="text-sm text-gray-500 mt-2">受付からヤードへの移動を管理します。加工記録は「ナゲット製造」タブで行います。</p>
        </div>
                ...
                return (
                  <div key={res.id} draggable onDragStart={(e) => onDragStart(e, res.id)} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm cursor-grab active:cursor-grabbing hover:shadow-md transition group relative">
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                      ...
                      {col.id === 'COMPLETED' && (
                          <button onClick={() => handleArchive(res.id)} className="text-xs font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 px-3 py-2 rounded-lg border border-gray-200 transition flex items-center gap-1">
                              <Icons.Archive /> レコードを完了
                          </button>
                      )}
                    </div>
                  </div>
