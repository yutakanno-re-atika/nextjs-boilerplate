...
export const AdminProduction = ({ data, localReservations }: { data: any, localReservations: any[] }) => {
  ...
  // 省略: handleSkipSort, handleSortSubmit, openBlendModal, handleProcessSubmit はそのまま
  ...
  const handleProcessSubmit = async () => {
    ...
    const inW = parseFloat(processInputWeight); const outC = parseFloat(processOutputCopper); const ratio = ((outC / inW) * 100).toFixed(1);
    ...
    try {
        const payload = {
            ...
            materialName: mixedName.substring(0, 50), inputWeight: inW, outputCopper: outC,
            actualRatio: parseFloat(ratio), memo: `作業者: ${processWorker} | 元ロット: ${blendingLots.length}件 | ${processMemo}`
        };
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500 text-gray-900 pb-12 font-sans max-w-7xl mx-auto w-full relative" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
      ...
      <div className="print:hidden flex flex-col h-full">
          ...
          {/* モーダル群（省略せずに残す） */}
          {sortingLot && ( <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in"> {/*...前と同じ...*/} </div> )}
          {blendingLots.length > 0 && ( <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in"> {/*...前と同じ...*/} </div> )}
      </div>
                          ...
                          {productions.slice(-5).reverse().map((p: any, i: number) => (
                              <li key={i} className="py-2 flex justify-between items-center text-xs">
                                  <div>
                                      ...
                                      <p className="text-[10px] text-gray-500">{String(p.date).substring(5,16)}</p>
                                  </div>
                                  <div className="text-right">
